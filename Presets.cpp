...
namespace Presets
{
	const Data kPresets[] = 
	{
		...
		{ "nonsense can", "(1 + $(m)%32) ^ (t*128 & t*64 & t*32) | (p/16)<<p%4 | $(p/128)>>p%4", 50, 15 },
		{ "ellipse", "(m/250+1)*$(t*128) | (m/500+1)*$((t+r/2*128))", 50, 12 },
		{ "moving average", "p + ( ((t+1)*256 ^ (t+1)*64 & (t+1)*32) - p)/(t+1)", 50, 15 },
		{ "oink oink ribbit", "(t*128 | t*17>>2) | ((t-4500)*64 | (t-4500)*5>>3) | p<<12", 30, 18 },
		{ "rhythmic glitch sine", "$(t*Fn) | t*n/10>>4 ^ p>>(m/250%12)", 15, 13 }
	};
