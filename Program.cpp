...
static int ParsePOK(CompilationState& state)
{
	...
	for (;;)
	{
		...
		state.parsePos++;
		// PEK works by popping a value from the stack to use as the lookup address.
		// so when we want to POK, we can use that same address to know where in memory to assign the result of the right side.
		// we just need to remove the existing PEK instruction so the address will still be on the stack
		// after the instructions generated by ParseTRN complete.
		// we require the presence of a PEK instruction because we don't want to allow statements like '5 = 4'
